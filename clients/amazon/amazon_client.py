...
class AmazonClient(InitDriver):
    ...
    def _get_single_photo(self, num_image=0):
        div = self.webdriver.find_element(By.XPATH, f"//div[@id='altImages']")
        ul = div.find_element(
            By.XPATH,
            f"//ul[@class='a-unordered-list a-nostyle a-button-list "
            f"a-vertical a-spacing-top-extra-large {CssClasses.LAYOUTS}']",
        )
        li = ul.find_elements(
        ...
        for image in li:
            ...
            hover = ActionChains(self.webdriver).move_to_element(span)
            hover.perform()
    ...
    def _with_alibaba(self, num_image):
        ...
        div = self.webdriver.find_element(By.XPATH, path)
    ...
    def _generate_path_for_image(num_image):
        return f"//li[@class='image item itemNo{num_image} maintain-height selected']" \
               ...
               f"/div[@class='{CssClasses.IMAGE_WRAPPER}']/img[@class='a-{CssClasses.MAIN_IMAGE}']"
