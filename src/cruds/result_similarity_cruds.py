...
class ResultSimilarityCRUDS:
    ...
    @staticmethod
    def insert_result_similarity(amazon_product_id: UUID, alibaba_product_id: UUID, similarity: float):
        most_similar_id = uuid.uuid4()

        alibaba_product = MostSimilarModel(id=most_similar_id,
    ...
    def remove_result_similarity(similarity_id: UUID):
        return (session.query(MostSimilarModel)
                .filter(MostSimilarModel.id == similarity_id).delete())
